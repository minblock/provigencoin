...
const char * const BITCOIN_CONF_FILENAME = "provigencoin.conf";
...
#else
    const char* pszModule = "provigencoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "provigencoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/provigencoin";
#else
    // Unix
    return pathRet / ".provigencoin";
#endif
#endif
}
