...
- [Development guidelines](#development-guidelines)
    - [General provigencoin Core](#general-provigencoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
provigencoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play provigencoins" on the test network, if you
...
provigencoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/provigencoin-project/provigencoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_provigencoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_provigencoin --log_level=test_suite
$ valgrind -v --leak-check=full src/provigencoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_provigencoin.coverage/index.html`.
```
...
To profile a running provigencoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep provigencoind` -- sleep 60
```
...
provigencoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as provigencoin Core, where everyone uses
...
pay attention to for reviewers of provigencoin Core code.

General provigencoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: provigencoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of provigencoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against provigencoin Core so that they can be integrated
...
file descriptors it will cause problems with provigencoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to provigencoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `provigencoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `provigencoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of provigencoin Core, the wallet was always
